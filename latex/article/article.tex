...
The previous sections introduced specialist agents and tools for model research, dataset inspection, runtime checks, and notebook prototyping. The Antigravity SDK sits beside those components as an opt-in orchestrator tool. It is exposed to the root orchestrator agent, so either the user or the orchestrator can call Antigravity to inspect or oversee artifacts produced by other agents, or when a request calls for stronger reasoning capabilities. Antigravity can be used for image generation, drafting validation reports, inspecting project structure, improving code quality, or producing a proposed notebook or script.

Antigravity can be used from the CLI, directly inside the Antigravity IDE, or through the SDK. This implementation uses the SDK because it exposes an API for programmatically interacting with the Antigravity platform. The SDK is designed for prototyping custom agents on top of that harness with minimal code, and the documentation positions it around simple Python scripts for iterating on agentic applications, automating software engineering tasks, and running evaluations. In \texttt{visionops\_crew}, that makes Antigravity useful for overseeing artifacts generated by other agents and for implementation-heavy tasks that require more multi-step reasoning.

The implementation in \texttt{tools.py} shows the SDK pattern used by the app. First, the tool builds a local Antigravity configuration from resolved workspace and artifact paths:

\begin{lstlisting}[language=Python, caption={Creating the local Antigravity SDK configuration in \texttt{tools.py}.}, label={lst:antigravity_config}]
...
Then \texttt{run\_antigravity\_agent} snapshots the artifact roots, executes the task as an asynchronous chat turn, and diffs those roots after the run:

\begin{lstlisting}[language=Python, caption={Running Antigravity from the orchestrator tool.}, label={lst:antigravity_run}]
artifact_hook = ArtifactTrackingHook()
roots = _artifact_roots(
    paths=paths,
    include_workspace=allow_file_edits,
)
...
\texttt{run\_antigravity\_agent} resolves its root folder from the caller or environment, loads \texttt{.env}, creates a visible \texttt{antigravity-workspace/}, and stores SDK state under \texttt{.antigravity/}. It also attaches a post-tool-call hook so the orchestrator receives Antigravity tool results, then reports created and updated files from the artifact diff.

The policy boundary is deliberately narrow. File creation and editing are denied by default and only enabled when \texttt{allow\_file\_edits} is true after explicit user confirmation. Shell command execution is denied in the non-interactive tool path. Image generation and other non-write tools remain available. This makes Antigravity a controlled extension of \texttt{visionops\_crew}, not a hidden side channel.
